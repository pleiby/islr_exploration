...
y <- rep(c(-1, 1), c(10, 10))
x[y == 1, ] <- x[y == 1, ] + 1 # shift all x obs for which y==1
plot(x, col = y + 3, pch = 19)
```

Load the package `e1071` which contains the `svm` function we will use. We then compute the fit. Notice that we have to specify a `cost` parameter, which is a tuning parameter. 
...
make.grid <- function(x, n = 75) {
  grange <- apply(x, 2, range)  # range returns vector containing min and max of all the given args.
  x1 <- seq(from = grange[1, 1], to = grange[2, 1], length = n) # construct uniform sequence across range
  x2 <- seq(from = grange[1, 2], to = grange[2, 2], length = n)
  expand.grid(X1 = x1, X2 = x2) # Create data frame from all combinations of the supplied vectors
}
```

```{r}
xgrid <- make.grid(x)
ygrid <- predict(svmfit, xgrid)  # classifies each point on xgrid as 1 or 2

# plot with point symbols selected by pch argument from this set:
#   http://www.sthda.com/english/wiki/r-plot-pch-symbols-the-different-point-shapes-available-in-r
#   Point size controlled by cex
...
Support vectors (outlined with black diamonds) are either on the margin lines
or violate them.
...
names(ESL.mixture)
rm(x, y)
attach(ESL.mixture) # allows referencing list elements w/o prefixing by list name
```
...
# svm produces a list of ~30 elements relate to the svm fit
...
```{r}
xgrid <- expand.grid(X1 = px1, X2 = px2) # px1 and px2 are ESL.mixture vars
...
func <- attributes(func)$decision  # yields numeric vector of decision fn vlues
